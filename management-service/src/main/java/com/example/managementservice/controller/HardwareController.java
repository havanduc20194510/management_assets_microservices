...
public class HardwareController {
    ...
    @PostMapping("/create")
    public ResponseEntity<Boolean> createHardware(@RequestBody @Valid HardwareDto hardware) {
        if (hardwareService.insertHardware(hardware)) {
    ...
    @GetMapping("/list")
    public Page<HardwareDto> getAllHardware(@RequestParam(name = "page", defaultValue = "1") int page, @RequestParam(name = "size", defaultValue = "2") int size) {
        return hardwareService.getAllPaginated(page, size);
    }
