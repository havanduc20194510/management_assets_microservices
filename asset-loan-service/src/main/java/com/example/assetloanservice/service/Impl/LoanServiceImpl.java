...
import com.example.assetloanservice.dto.*;
...
public class LoanServiceImpl implements LoanService {
    ...
    @Override
    public Page<LoanDetailResponseDto> getAllLoansListByUserName(String username, int page, int size) {
        Pageable pageable = PageRequest.of(page-1, size);
        Page<Loans> loansPage = loansRepository.findAllByUsernameOrderByStartDateDesc(username, pageable);
        List<LoansDto> loanDTOs = loansPage.getContent().stream()
                .map(loans -> loansMapper.toDto(loans))
                .toList();
        List<LoanDetailResponseDto> loanDetailResponseDtos = new ArrayList<LoanDetailResponseDto>();
        loanDTOs.forEach(loanDto -> {
            List<HardwareDto> hardwareDtos = new ArrayList<>();
            loanDto.getDetails().forEach(hardware -> {
                HardwareDto hardwareDto = apiClient.getHardwareDto(hardware.getAssetCode());
                hardwareDto.setQuantity(hardware.getQuantity());
                hardwareDtos.add(hardwareDto);
            });
            LoanDetailResponseDto loanDetailResponseDto = new LoanDetailResponseDto();
            loanDetailResponseDto.setId(loanDto.getId());
            loanDetailResponseDto.setUsername(loanDto.getUsername());
            loanDetailResponseDto.setStartDate(loanDto.getStartDate());
            loanDetailResponseDto.setDueDate(loanDto.getDueDate());
            loanDetailResponseDto.setReturnDate(loanDto.getReturnDate());
            loanDetailResponseDto.setStatus(loanDto.getStatus());
            loanDetailResponseDto.setDetails(hardwareDtos);

            loanDetailResponseDtos.add(loanDetailResponseDto);
        });

        return new PageImpl<>(loanDetailResponseDtos, pageable, loansPage.getTotalElements());
    }
